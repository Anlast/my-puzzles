 ...
 * 
 * TODO:
 * - the solver and generator will be remade with latin.h
 */
...
static bool solver_elim_farpos_inrow(struct solver_usage *usage, const struct edges *edges, int row) {
	bool ret = false;
	int i, j, k, min, max, val, s;
	int *indices, *iter;
	
	if(edges->right[row] == 0) /* temporary, prob goto left ? */
		return false;
	
	indices = malloc(sizeof(int)*usage->par.wh*usage->par.max);
	for(i = 0; i < usage->par.wh*usage->par.max; i++)
		indices[i] = -1;
	
	for(j = 0; j < usage->par.max; j++)
	for(i = k = 0; i < usage->par.wh; i++)
	{
		if((1 << j) & usage->values[usage->par.wh*row+i])
		{
			indices[usage->par.wh*j + k] = i;
			k++;
		}
	}
	
	for(j = 0; j < usage->par.max; j++) /* some values are already allocated */
	if(!(usage->row[row] & (1 << j)))
	{
		for(i = 0; i < usage->par.wh; i++)
		if(usage->grid[usage->par.wh*row + i] & (1 << j))
		{
			indices[usage->par.wh*j] = i;
			indices[usage->par.wh*j+1] = -1; /* this shouldn't be necessary */
			break;
		}
	}
		
	
	iter = malloc(sizeof(int)*usage->par.max);
	for(i = 0; i < usage->par.max; i++)
		iter[i] = -1; /* current poistion in each row */
	
	val = log2(edges->right[row]); /* skip the edge value */
	min = usage->par.wh;
	j = 0;
	if(j == val)
		j++;
	...
	while(j != -1) /* iteration */
	{
		do
		{
			...
			for(i = 0; i < j; i++)
			{
				...
				if(indices[usage->par.wh*i+iter[i]]	== indices[usage->par.wh*j+iter[j]])
				{
					valid = false;
					break;
				}
		...
		} while(indices[usage->par.wh*j + iter[j]] != -1);
		if(indices[usage->par.wh*j + iter[j]] == -1 || indices[usage->par.wh*j+iter[j]] > min)
		{
			iter[j] = -1;
			j--;
			if(j == val)
		...
		if(s == usage->par.max-1) /* built the whole sequence, time to save the result */
		{	
			int m;
			max = -1;
			for(i = 0; i < usage->par.max; i++)
			{
				...
				if((m = indices[usage->par.wh*i+iter[i]]) > max)
					max = m;
			}
			...
			if(min == usage->par.max-2) /* the shortest path possible */
				break;
		}
					
		if(j < usage->par.max-1)
		{
			j++;
			if(j == val)
				j += (j < usage->par.max-1) ? +1 : -1;
		}
		
	}

	//assert(min != usage->par.wh);
	#if 1
	if(min == usage->par.wh)
		printf("error at %d! min:%d\n", row, min);
	#endif
	
	//printvalues(usage);
	
	for(i = 0; i <= min; i++)
	if(solver_ispossible((1 << val), usage->values[usage->par.wh*row + i]))
	{
		#if 1
		printf("min:%d\n", min);
		#endif
		ret = true;
		solver_removeval(usage->par.wh*row + i, (1 << val));
		#ifdef STANDALONE_SOLVER
		if(solver_show_working && verbose)
			printf("far pos elim(from left) %d: at (%d, %d)\n", (1 << val), row, i);
		#endif
	}
	...
	---------------------------------------------------- */
	#if 0
	/* reverse the order of all indices and in the matrix */
	for(j = 0; j < usage->par.wh; j++)
	{
		int temp;
		i = 0;
		while(indices[usage->par.wh*j+i+1] != -1)
		{
			temp = indices[usage->par.wh*j];
			indices[usage->par.wh*j+i] = indices[usage->par.wh*j+i+1];
			indices[usage->par.wh*j+i+1] = temp;
		}
	}
	for(j = 0; j < usage->par.wh; j++)
	{
		int temp;
		for(i = 0; i+1<usage->par.wh; i++)
		{
			temp = matrix[usage->par.wh*j+i];
			matrix[usage->par.wh*j+i] = matrix[usage->par.wh*j+i+1];
			matrix[usage->par.wh*j+i+1] = temp;
		}
	}
	
	iter = malloc(sizeof(int)*usage->par.max);
	for(i = 0; i < usage->par.max; i++)
		iter[i] = 0; /* current poistion in each row */
	
	val = log2(edges->left[row]); /* skip the edge value */
	min = usage->par.wh;
	j = 0;
	if(j == val)
		j++;
	...
	while(j != -1) /* dfs */
	{
		while(indices[iter[j]] != -1)
		{
			for(i = 0; i < usage->par.max; i++)
			if(matrix[usage->par.wh*j + indices[iter[j]]] && val != i)
				break;
			
			if(i == usage->par.max)	/* valid idx */
			{
				s++;
				break;
			}
		...
		if(indices[iter[j]] == -1 || indices[iter[j]] > min)
		{
			iter[j] = 0;
			j--;
			if(j == val)
		...
		{			
			for(i = 0; i < usage->par.max; i++)
			if((usage->par.wh-indices[iter[i]]) > max) /* ---------------- reverse len calculation ----------- */
				max = (usage->par.wh-indices[iter[i]]);
	...
	for(i = usage->par.wh-1; i > usage->par.wh-min; i--)
	if(solver_ispossible(val, usage->values[usage->par.wh*row + i]))
	{
		ret = true;
		solver_removeval(usage->par.wh*row + i, val);
		#ifdef STANDALONE_SOLVER
		if(solver_show_working && verbose)
			printf("far pos elim(from right) %d: at (%d, %d)", val, row, i);
		#endif
	}
	#endif
	/* ---------------------------------------------------- */
